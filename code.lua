...
---------------------------------------------------------
-- Вкладка: Illegal Football (Repotted) ULTIMATE
---------------------------------------------------------
local FootballTab = Window:CreateTab("⚽ Illegal Football", 4483362458)

-- Переменные для функций
local InfiniteStamina = false
local AutoTackle = false
local BallMagnet = false
local GoalSilentAim = false
local AutoKeeper = false
local BallESP = false

-- Функция поиска мяча на карте
local function GetBall()
    return workspace:FindFirstChild("Ball") 
        or workspace:FindFirstChild("SoccerBall") 
        or workspace:FindFirstChild("Football") 
        or workspace:FindFirstChild("Meshes/SoccerBall")
end

-- Функция поиска вражеских ворот
local function GetEnemyGoal()
    local myTeam = LocalPlayer.Team
    for _, obj in pairs(workspace:GetDescendants()) do
        if (obj.Name:find("Goal") or obj.Name:find("Net")) and obj:IsA("BasePart") then
            -- Если есть разделение по командам, ищем чужие ворота
            if myTeam and not obj.Name:lower():find(myTeam.Name:lower()) then
                return obj
...
local FBVisuals = FootballTab:CreateSection("Визуалы и Трэкинг")

FootballTab:CreateToggle({
    Name = "Ball ESP (Подсветка и Tracer к мячу)",
    CurrentValue = false,
    Flag = "FBBallESP",
    Callback = function(Value)
        BallESP = Value
        local ball = GetBall()
        if not Value and ball and ball:FindFirstChild("BallHighlight") then
            ball.BallHighlight:Destroy()
        end
    end
})

local FBPhys = FootballTab:CreateSection("Физика и Персонаж")

FootballTab:CreateToggle({
    Name = "Infinite Stamina (Бесконечный спринт)",
    CurrentValue = false,
    Flag = "FBInfStamina",
    Callback = function(Value)
...
FootballTab:CreateSlider({
    Name = "Скорость бега (Speed Boost)",
    Range = {16, 50},
    Increment = 1,
    Suffix = "Speed",
    CurrentValue = 16,
...
local FBBallSec = FootballTab:CreateSection("Работа с Мячом и Аим")

FootballTab:CreateToggle({
    Name = "Ball Magnet / Reach (Мяч липнет к ногам)",
    CurrentValue = false,
    Flag = "FBBallMagnet",
    Callback = function(Value)
        BallMagnet = Value
...
FootballTab:CreateToggle({
    Name = "Auto-Tackle (Авто-отбор/подкат)",
    CurrentValue = false,
    Flag = "FBAutoTackle",
    Callback = function(Value)
        AutoTackle = Value
...
FootballTab:CreateToggle({
    Name = "Goal Silent Aim (Авто-наводка удара в ворота)",
    CurrentValue = false,
    Flag = "FBGoalAim",
    Callback = function(Value)
        GoalSilentAim = Value
...
FootballTab:CreateToggle({
    Name = "Auto-Keeper (Авто-сейв для Вратаря)",
    CurrentValue = false,
    Flag = "FBAutoKeeper",
    Callback = function(Value)
        AutoKeeper = Value
...
---------------------------------------------------------
-- ФОНОВАЯ ЛОГИКА И ЦИКЛЫ
---------------------------------------------------------

-- 1. Подсветка мяча (ESP)
task.spawn(function()
    while task.wait(0.5) do
        if BallESP then
            local ball = GetBall()
...
-- 3. Основной игровой цикл (RenderStepped)
RunService.RenderStepped:Connect(function()
    local char = LocalPlayer.Character
    if not char or not char:FindFirstChild("HumanoidRootPart") then return end
    local hrp = char.HumanoidRootPart
    ...
    if ball and ball:IsA("BasePart") then
        ...
        -- Magnet / Reach (Подтягиваем мяч, если близко)
        if BallMagnet and distance < 15 then
            if firetouchinterest then
        ...
        -- Auto-Tackle (Авто-подкат)
        if AutoTackle and distance < 8 then
            if mouse1click then mouse1click() end
            -- Запасной вариант эмуляции клавиши подката (E)
        ...
        -- Goal Silent Aim (Камера смотрит на ворота при ударе)
        if GoalSilentAim and distance < 12 then
        ...
        -- Auto-Keeper (Телепорт ноги к мячу в штрафной)
        if AutoKeeper and distance < 25 and distance > 3 then
            local speed = ball.AssemblyLinearVelocity.Magnitude
            if speed > 10 then -- Если мяч летит быстро в сторону игрока
